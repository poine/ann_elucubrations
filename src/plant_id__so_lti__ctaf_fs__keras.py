...
class ANN_Plant:
    ...
    def __init__(self):
        _i1 = keras.layers.Input(shape=(2,)) # x1_km1, x2_km1
        _i2 = keras.layers.Input(shape=(1,)) # u_km1
        _tf = keras.layers.Dense(2, input_shape=(2,), use_bias=False, name='f')(_i1)
        _tg = keras.layers.Dense(2, input_shape=(2,), use_bias=True, name='g')(_i1)
        _t_ug = keras.layers.multiply([_i2, _tg])
        _t_xkp1 = keras.layers.add([_tf, _t_ug])
        self.ann = keras.models.Model(inputs=[_i1, _i2], outputs=_t_xkp1)
        self.ann.compile(loss='mean_squared_error', optimizer='adam', metrics=['accuracy'])
        self.ann.summary()

    def make_input(self, X, U):
        _i1 = np.zeros(( len(X)- self.delay, 2))
        _i2 = np.zeros(( len(X)- self.delay, 1))
        for i in range(self.delay, len(X)):
            _i1[i-self.delay, 0] = X[i-1, 0]
            _i1[i-self.delay, 1] = X[i-1, 1]
            _i2[i-self.delay, 0] = U[i-1]
        return _i1, _i2

    def fit(self, time, X, U):  
        print('building training set')
        (ann_input1, ann_input2), ann_output = self.make_input(X, U), X[self.delay:]
        print('fiting set')
    ...
    def get(self, x1_km1, x2_km1, u_km1):
        return self.ann.predict([np.array([[x1_km1, x2_km1]]), np.array([[u_km1]])])
...
def main(make_training_set=True, train=True, test=True):
    ann_plant_filename = '/tmp/so_lti__plant_id__ctaf_fs__sklearn.pkl'
    ...
    if test:
        ...
        plt.savefig('../docs/plots/plant_id__so_lti__ctaf_fs__keras.png')
        plt.show()
