...
class ANN_Plant:
    delay = 2
    th_km1, th_km2, v_km1, v_km2, input_size = range(5)
    ...
    def make_input(self, X, U):
        ...
        _output = X[self.delay:, 0]
        for i in range(self.delay, len(X)):
            _input[i-self.delay, self.th_km1] = X[i-1, robot_arm.Plant.s_phi]
            _input[i-self.delay, self.th_km2] = X[i-2, robot_arm.Plant.s_phi]
            _input[i-self.delay, self.v_km1] = U[i-1, robot_arm.Plant.i_tau]
            _input[i-self.delay, self.v_km2] = U[i-2, robot_arm.Plant.i_tau]
        return _input, _output

    
    def fit(self, time, X, U):  
        print('building training set')
        ann_input, ann_output = self.make_input(X, U)
    ...
    def summary(self):
        #pdb.set_trace()
        if 0: # LTI SSE when no internal layer
    ...
    def sim(self, time, X0, ctl):
        ...
        for i in range(1, len(time)):
            ...
            _input = [[X[i-1,0], X[i-2,0], U[i-1,0], U[i-2,0]]]
            X[i,0] = self.ann.predict(_input)
        U[-1] = U[-2]
        return X, U       
...
def main(make_training_set=True, train=True, test=True):
    ...
    ann_plant_filename = '/tmp/robot_arm__plant_id__io_ann.pkl'
    traj_filename = '/tmp/robot_arm__plant_id__training_traj.pkl'
    
    plant = robot_arm.Plant()
    ann = ANN_Plant()
    if train:
        time, X, U, desc = robot_arm.make_or_load_training_set(plant, make_training_set, traj_filename, nsamples=int(150*1e3))
    ...
    if test:
        ...
        plt.savefig('../docs/plots/plant_id__robot_arm__io__sklearn.png')
        plt.show()
